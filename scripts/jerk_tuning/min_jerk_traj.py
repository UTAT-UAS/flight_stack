...
class BTreeFlightPlanner(FlightPlanner):
    ...
    def inner_current_loop(self):
        ...
        vel_ratio = max(0.0, min(vel_ratio, 1.0)) # Clamp between 0 and 1 # abs() **-1 if > 1 instead to handle actual v > target v?
        ...
        # current-based target vel
        self.cruise_target_velocity = self.base_ff_velocity + p_term + self.stored_integral
        self.expected_cruise_velocity = self._manual_ema(self.cruise_target_velocity, self.expected_cruise_velocity, 0.01 + 0.03*vel_ratio)

        # projection and curvature
        self.pathtime = self.projection()
        min_jerk_scale = self.velocity_scale()
        self.min_jerk_velocity = min_jerk_scale * self.expected_cruise_velocity
        self.target_velocity = min(self.cruise_target_velocity, self.min_jerk_velocity)
    ...
    # parameters
    T = 3
    expected_v_cruise = 15
    horizon = T * expected_v_cruise
    resolution = 0.5
    project_ahead = 1
    ...
    def velocity_scale(self) -> float:
        ...
        sum_v_x = 0
        sum_v_y = 0
        for i, dt in enumerate(np.arange(-horizon + project_ahead, project_ahead, resolution)):
            pos0 = self.traj.path(self.pathtime + dt)
            pos1 = self.traj.path(self.pathtime + dt + horizon)
            vel0 = self.traj.velocity(self.pathtime + dt)
            vel1 = self.traj.velocity(self.pathtime + dt + horizon)
