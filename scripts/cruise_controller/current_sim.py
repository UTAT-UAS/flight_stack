...
class CurrentSim(Node):
    ...
    def _velocity_cb(self, msg: VehicleLocalPosition) -> None:
        self._v_mag = math.sqrt(msg.vx**2 + msg.vy**2 + msg.vz**2)
    ...
    def _estimate_current_draw(self) -> float:
        ...
        x = [(motor - 0.22) * 100 for motor in self._motor_outputs]  # adjust for cyclone type values
        i_motor = [
    ...
    def _publish_current_draw(self) -> None:
        message = Float32()
        message.data = self._estimate_current_draw()
        self._current_publisher.publish(message)
